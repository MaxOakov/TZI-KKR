...
void draw() {
  image(pg, 0, 0);    // Вивід картинки в вікно програми
}

// отримуємо ядро з ключа шифрування
int getSeed() {  
  String thisKey = cp5.get(Textfield.class, "key").getText(); // нехай ключем буде слово "key"
  int keySeed = 1;
  for (int i = 0; i < thisKey.length(); i++) keySeed *= int(thisKey.charAt(i));  // перемножуванням   
  return keySeed;
}

// отримати незайняте випадкове число
...
void encrypt() {
  if (refPath.length() != 0 && imagePath.length() != 0) {    
    ...
    int framerate = int(file.frames() / file.duration());           // частота оцифровування
    loadSound(file, pcmRef);

    PImage image = loadImage(imagePath);
    image.loadPixels();
    if (pcmRef.length < (image.width * image.height) * 3 * 2 + 4) {  // 3 байти, через фрейм + 4 службових
...
void decrypt() {
  if (cryptPath.length() != 0) {
    ...
    decrypt.updatePixels();                 // оновлюємо зображення
    decrypt.save("decrypt_image.bmp");      // зберігаємо
    debugArea.setText("Finished");

    // вивести у вікно програми
    pg.beginDraw();
    pg.background(130);
    int maxW = width-130;
    int maxH = height-20;
...
// завантажити файл (моно або стерео) в pcmRef
void loadSound(SoundFile file, byte[] pcm) {
  float floatData[] = new float [file.frames() * file.channels()];  // повний масив  
  file.read(floatData);                                             // читаємо в масив
